...
import { PremiumMarketplaceHome } from "@/components/marketplace/premium-marketplace-home";
...
import { getMarketplaceSnapshot } from "@/server/services/marketplace.service";
...
export default async function Home() {
  ...
  const tenantMeta = tenantCtx?.tenantId
  ...
  const isMarketplaceMode = !tenantMeta || tenantMeta.type === "PLATFORM" || tenantMeta.isPlatform === true;
  ...
  if (isMarketplaceMode) {
    ...
    return (
      <>
        ...
        <PremiumMarketplaceHome snapshot={snapshot} />
      </>
    );
  }

  const publishedHome = await getPublishedPage({
          ...
          <>
            <section className="grid gap-8 py-16 lg:grid-cols-[1.15fr_0.85fr]">
              <div className="space-y-5">
                ...
                <h1 className="text-4xl font-semibold tracking-tight text-slate-900 md:text-6xl">{t.homeTitle}</h1>
                <p className="max-w-2xl text-base text-slate-700 md:text-lg">{demoTenant.seoDescription}</p>
